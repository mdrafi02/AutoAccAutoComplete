...
def train_model(json_path, context_size=2, batch_size=512, epochs=30, 
                validation_split=0.2, embedding_dim=64, lstm_units=64,
    ...
    if continue_training and existing_tokenizer_path and os.path.exists(existing_tokenizer_path):
        ...
        if new_vocab_size > old_vocab_size:
            print(f"📈 Vocabulary expanded: {old_vocab_size} → {new_vocab_size} (+{new_vocab_size - old_vocab_size} new words)")
        else:
        ...
        try:
            ...
            if vocab_size > old_vocab_size:
                ...
                if model.input_shape[1] != context_size:
                    print(f"⚠️  Context size mismatch! Model expects {model.input_shape[1]}, got {context_size}")
    ...
    if model is None or vocab_expanded:
        ...
        if continue_training and existing_model_path and os.path.exists(existing_model_path) and not vocab_expanded:
            try:
                ...
                for i, (old_layer, new_layer) in enumerate(zip(old_model.layers, model.layers)):
                    if i == 0:  # Embedding layer - only copy if vocab size matches
                        if old_layer.input_dim == new_layer.input_dim:
                            ...
                            print(f"   ✅ Copied weights from layer {i} ({old_layer.__class__.__name__})")
    ...
    if len(X_train) > 100000:
        ...
        train_gen = data_generator_cached(X_train, y_train, batch_size=batch_size, shuffle=True)
        
        if X_val is not None:
            val_steps = ceil(len(X_val) / batch_size)
            val_gen = data_generator_cached(X_val, y_val, batch_size=batch_size, shuffle=False)
            history = model.fit(
                train_gen,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train LSTM model for keyword prediction")
    parser.add_argument("--input", "-i", type=str, default="keyword_dataset_cleaned.json",
                       help="Input JSON dataset file")
    parser.add_argument("--context-size", "-c", type=int, default=2,
    ...
    parser.add_argument("--model-output", "-m", type=str, default="keyword_predictor.keras",
                       help="Output model file path")
    parser.add_argument("--tokenizer-output", "-t", type=str, default="tokenizer.json",
    ...
    parser.add_argument("--existing-tokenizer", type=str,
