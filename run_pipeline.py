...
Examples:
  ...
  python run_pipeline.py --xml-folder /path/to/xml \\
    ...
    # Input/Output paths
    parser.add_argument("--xml-folder", "-x", type=str, required=True,
                       help="Folder containing Robot Framework XML files")
    parser.add_argument("--output-dataset", "-o", type=str, 
                       default="keyword_dataset.json",
                       help="Output path for extracted dataset")
    parser.add_argument("--cleaned-dataset", "-c", type=str,
                       default="keyword_dataset_cleaned.json",
                       help="Output path for cleaned dataset")
    parser.add_argument("--model-output", "-m", type=str,
                       default="keyword_predictor.keras",
                       help="Output path for trained model")
    parser.add_argument("--tokenizer-output", "-t", type=str,
    ...
    if "extract" in steps_to_run:
        cmd = [
            sys.executable, "extract_keywords.py",
            "--folder", args.xml_folder,
    ...
    if "inspect" in steps_to_run:
        cmd = [
            sys.executable, "inspect_keyword_dataset.py",
            "--file", args.output_dataset
        ]
    ...
    if "clean" in steps_to_run:
        cmd = [
            sys.executable, "clean_keyword_dataset.py",
            "--input", args.output_dataset,
            "--output", args.cleaned_dataset
        ]
    ...
    if "test" in steps_to_run or args.interactive_test:
        ...
        cmd = [
            sys.executable, "predict_keywords.py",
            "--model", args.model_output,
            "--tokenizer", args.tokenizer_output
        ]
        print(f"\n{'='*60}")
