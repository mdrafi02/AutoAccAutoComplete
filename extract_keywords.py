...
def extract_keywords_from_output(xml_path):
    ...
    try:
        ...
        for event, element in ET.iterparse(xml_path, events=("start", "end")):
            if event == "start":
                if element.tag == "suite":
                    ...
                    current_suite_id = element.attrib.get("id", f"suite_{len(suite_keywords_map)}")
                    if current_suite_id not in suite_keywords_map:
                        suite_keywords_map[current_suite_id] = {"setup": [], "teardown": []}
                    ...
                    is_suite_setup = (not in_suite_setup and not in_suite_teardown and 
                                     kw_type == "SETUP" and not current_test_name)
                    is_suite_teardown = (not in_suite_setup and not in_suite_teardown and 
                                        kw_type == "TEARDOWN" and not current_test_name)
                    is_test_setup = kw_type == "SETUP" and current_test_name and not in_test_setup and not in_test_teardown
                    is_test_teardown = kw_type == "TEARDOWN" and current_test_name and not in_test_setup and not in_test_teardown
                    ...
                    if is_suite_setup:
                        ...
                        suite_setup_depth = 0
                        current_suite_setup_keywords = []
                        suite_setup_keyword_name = normalized_name  # Store the keyword name
                    elif is_suite_teardown:
                        # Suite-level teardown (applies to all tests in suite)
                        in_suite_teardown = True
                        suite_teardown_depth = 0
                        current_suite_teardown_keywords = []
                        suite_teardown_keyword_name = normalized_name  # Store the keyword name
                ...
                if element.tag == "test":
                    if current_test_name:
                        ...
                        if current_suite_id and current_suite_id in suite_keywords_map:
                            suite_setup_keywords = suite_keywords_map[current_suite_id]["setup"]
                            suite_teardown_keywords = suite_keywords_map[current_suite_id]["teardown"]
                    ...
                    if current_suite_id and current_suite_id in suite_tests_map:
                        suite_teardown_keywords = suite_keywords_map.get(current_suite_id, {}).get("teardown", [])
                        ...
                        for test_dict in suite_tests_map[current_suite_id]:
                            ...
                            if suite_teardown_keywords:
                                ...
                                if len(keywords) >= len(suite_teardown_keywords):
                                    if keywords[-len(suite_teardown_keywords):] != suite_teardown_keywords:
                    ...
                    keyword_name = element.attrib.get("name", "")
                    kw_type = element.attrib.get("type", "").upper()
                    normalized_name = keyword_name.lower().strip() if keyword_name else ""
                    ...
                    is_suite_setup_end = (in_suite_setup and suite_setup_depth == 0 and 
                                         (normalized_name == suite_setup_keyword_name or 
                                          (kw_type == "SETUP" and not current_test_name)))
                    is_suite_teardown_end = (in_suite_teardown and suite_teardown_depth == 0 and 
                                            (normalized_name == suite_teardown_keyword_name or 
                                             (kw_type == "TEARDOWN" and not current_test_name)))
                    ...
                    if is_suite_setup_end:
                        ...
                        if current_suite_id:
                            suite_keywords_map[current_suite_id]["setup"] = current_suite_setup_keywords.copy()
                        ...
                        if current_suite_id:
                            suite_keywords_map[current_suite_id]["teardown"] = current_suite_teardown_keywords.copy()
        ...
        for suite_id, tests in suite_tests_map.items():
            suite_teardown_keywords = suite_keywords_map.get(suite_id, {}).get("teardown", [])
        ...
        has_suite_setup = any(len(suite_keywords_map.get(sid, {}).get("setup", [])) > 0 
                             for sid in suite_keywords_map.keys())
        has_suite_teardown = any(len(suite_keywords_map.get(sid, {}).get("teardown", [])) > 0 
                                for sid in suite_keywords_map.keys())
    ...
    for idx, xml_file in enumerate(xml_files, 1):
        ...
        if all_tests:
            ...
            if len(all_tests) > 0:
                ...
                print(f"   Sample keywords ({len(sample_keywords)}): {sample_keywords[:5]}{'...' if len(sample_keywords) > 5 else ''}")
                # Generic check: Use metadata from extraction (not keyword name patterns)
                print(f"   Suite setup detected: {'Yes ✓' if has_suite_setup else 'No'}")
                print(f"   Suite teardown detected: {'Yes ✓' if has_suite_teardown else 'No'}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract keyword sequences from Robot Framework XML logs")
    ...
    parser.add_argument("--output", "-o", type=str, default="keyword_dataset.json",
    ...
    try:
        ...
        if args.append or args.merge:
            ...
            if args.merge:
                ...
                existing_set = {(item.get("test_name", ""), tuple(item.get("keywords", []))) 
                               for item in existing_data}
                new_set = {(item.get("test_name", ""), tuple(item.get("keywords", []))) 
                ...
                print(f"🔄 Merged: {added} new test cases added, {len(existing_data)} total")
            else:
                # Simple append
                all_data = existing_data + new_data
                print(f"➕ Appended: {len(new_data)} new test cases, {len(all_data)} total")
        else:
            all_data = new_data
