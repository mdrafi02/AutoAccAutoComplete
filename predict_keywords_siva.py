...
def predict_next_keyword(model, tokenizer, context_keywords, top_k=3):
    ...
    if len(context_keywords) > context_size:
        ...
        context_keywords = [""] * (context_size - len(context_keywords)) + context_keywords
...
def predict_next_keyword_with_depth(model, tokenizer, context_keywords, top_k=3, depth=1):
    """
    ...
    current_predictions = predict_next_keyword(model, tokenizer, context_keywords, top_k=top_k)
    ...
    for keyword, probability in current_predictions:
        ...
        if depth > 1:
            ...
            if len(new_context) >= context_size:
                new_context = new_context[-context_size+1:]  # Keep last (context_size-1)
...
def display_hierarchical_predictions(predictions, level=0, max_level=None):
    ...
    for i, pred in enumerate(predictions):
        ...
        if level == 0:
            print(f"{indent}{prefix} {i+1}. {keyword:45s} {bar} {probability*100:5.1f}%")
        else:
        ...
        if "next_predictions" in pred and pred["next_predictions"]:
            display_hierarchical_predictions(pred["next_predictions"], level + 1, max_level)
...
def cli_loop(model, tokenizer, depth=1):
    ...
    context_size = model.input_shape[1]
    sequence_history = []  # Store the full sequence of keywords for sequential prediction
    ...
    while True:
        try:
            ...
            if sequence_history:
                ...
                context_keywords = sequence_history[-context_size:] if len(sequence_history) >= context_size else sequence_history
                print(f"\n🎯 CONTEXT USED (last {len(context_keywords)}): {' → '.join(context_keywords)}")
                if len(sequence_history) > context_size:
                    print(f"   ⚠️  Note: Model uses only last {context_size} keywords (input shape limitation)")
                    print(f"   ℹ️  Model learned patterns from longer sequences during training")
            else:
            ...
            if len(sequence_history) == 0:
                ...
                if depth > 1:
                    hierarchical_predictions = predict_next_keyword_with_depth(model, tokenizer, context_keywords, top_k=3, depth=depth)
                    current_predictions = [(p["keyword"], p["probability"]) for p in hierarchical_predictions]
                    ...
                    current_predictions = predict_next_keyword(model, tokenizer, context_keywords)
