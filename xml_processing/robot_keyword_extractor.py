...
Part of the XML Processing module.
"""
...
class RobotKeywordExtractor:
    ...
    def export_to_json(self, output_file: str, indent: int = 2) -> None:
        ...
        with open(output_file, 'w', encoding='utf-8') as jsonfile:
            json.dump(json_data, jsonfile, indent=indent, ensure_ascii=False)
...
def main():
    ...
    parser.add_argument(
        'output_files',
        nargs='+',
        help='Path(s) to Robot Framework output.xml file(s). Can specify multiple files.'
    )
    parser.add_argument(
        '-d', '--details',
        action='store_true',
    ...
    parser.add_argument(
        '-o', '--output',
        help='Export keywords to file (CSV or JSON based on file extension). All files are combined.'
    )
    
    parser.add_argument(
        '-j', '--json',
        help='Export keywords to JSON file (alternative to -o with .json extension). All files are combined.'
    )
    ...
    all_keywords = []
    total_files = len(args.output_files)
    
    print(f"Processing {total_files} output.xml file(s)...")
    print("=" * 80)
    
    for i, output_file in enumerate(args.output_files, 1):
        print(f"\n[{i}/{total_files}] Processing: {output_file}")
        try:
    ...
    print(f"\nCombined total: {len(all_keywords)} keywords from {total_files} file(s)")
    
    # Create a combined extractor for output
    combined_extractor = RobotKeywordExtractor(args.output_files[0])
    combined_extractor.keywords = all_keywords
    
    # Print results
    combined_extractor.print_keywords(show_details=args.details, filter_status=args.status)
    
    # Export to file if requested
    if args.json:
        combined_extractor.export_to_json(args.json)
    elif args.output:
        # Detect format from file extension
        if args.output.lower().endswith('.json'):
            combined_extractor.export_to_json(args.output)
        else:
            combined_extractor.export_to_csv(args.output)
    
    # Print summary
    total_keywords = len(combined_extractor.keywords)
    if args.status:
        filtered_count = len([k for k in combined_extractor.keywords 
        ...
        print(f"\nSummary: {total_keywords} keywords found in execution order from {total_files} file(s).")
