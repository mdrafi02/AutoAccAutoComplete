...
from xml_processing.robot_keyword_extractor import RobotKeywordExtractor
...
class KeywordPatternAnalyzer:
    ...
    def _context_weights(self, context_len: int) -> List[float]:
        """Recent keywords receive higher weight (1.0 for last, decaying for earlier)."""
        return [0.8 ** (context_len - 1 - i) for i in range(context_len)]

    def _keywords_in_order(self, sequence: List[str], keywords: List[str]) -> int:
        """Return index of last matched keyword if all appear in order, else -1."""
        pos = 0
        last_idx = -1
        for kw in keywords:
            try:
                idx = sequence.index(kw, pos)
                last_idx = idx
                pos = idx + 1
            except ValueError:
                return -1
    ...
    def _score_next_keywords(self, matching_sequences: List[List[str]],
                             context_keywords: List[str],
                             max_recommendations: int) -> List[KeywordRecommendation]:
        """Score next keywords from matching sequences with weighted context."""
        weights = self._context_weights(len(context_keywords))
        next_keyword_scores = defaultdict(float)

        for sequence in matching_sequences:
            last_context_pos = self._keywords_in_order(sequence, context_keywords)
            if last_context_pos < 0 or last_context_pos + 1 >= len(sequence):
                continue
            ...
            for i, idx in enumerate(positions):
                recency = len(context_keywords) - 1 - i
                match_score += weights[recency] * (1.0 + idx / max(len(sequence), 1))

            next_keyword_scores[next_keyword] += match_score

        if not next_keyword_scores:
            return []

        total = sum(next_keyword_scores.values())
        recommendations = []
        for keyword, score in sorted(next_keyword_scores.items(),
                                     key=lambda x: x[1], reverse=True)[:max_recommendations]:
            ...
            recommendations.append(KeywordRecommendation(
                ...
                usage_count=int(score),
                next_keywords=[]
            ))
        return recommendations

    def _exact_context_match(self, context_keywords: List[str],
                             max_recommendations: int) -> List[KeywordRecommendation]:
        """Find sequences containing all context keywords (any positions)."""
        matching = [s for s in self.keyword_sequences
                    if all(kw in s for kw in context_keywords)]
        return self._score_next_keywords(matching, context_keywords, max_recommendations)

    def _fuzzy_context_match(self, context_keywords: List[str],
                             max_recommendations: int) -> List[KeywordRecommendation]:
        """Find sequences where context keywords appear in order (gaps allowed)."""
        matching = [s for s in self.keyword_sequences
                    if self._keywords_in_order(s, context_keywords) >= 0]
        return self._score_next_keywords(matching, context_keywords, max_recommendations)

    def _ngram_fallback(self, context_keywords: List[str],
                        max_recommendations: int) -> List[KeywordRecommendation]:
        """Fallback using progressively shorter suffix n-grams."""
        for n in range(min(5, len(context_keywords)), 0, -1):
            suffix = context_keywords[-n:]
            if n >= 2:
                matching = [s for s in self.keyword_sequences
                            if self._keywords_in_order(s, suffix) >= 0]
                recs = self._score_next_keywords(matching, suffix, max_recommendations)
                if recs:
                    for rec in recs:
                        rec.confidence *= 0.7 ** (len(context_keywords) - n)
                    return recs

            last_kw = suffix[-1]
            if last_kw in self.keyword_transitions:
                transitions = self.keyword_transitions[last_kw]
                total = sum(transitions.values())
                recommendations = []
                for keyword, count in transitions.most_common(max_recommendations):
                    library = self.keyword_libraries.get(keyword, 'BuiltIn')
                    recommendations.append(KeywordRecommendation(
                        keyword=keyword,
                        library=library,
                        confidence=(count / total) * (0.5 ** (len(context_keywords) - n)),
                        context=" -> ".join(context_keywords),
                        usage_count=count,
                        next_keywords=[]
                    ))
    ...
    def get_context_recommendations(self, context_keywords: List[str], 
                                  ...
                                  max_context_window: int = 10) -> List[KeywordRecommendation]:
        """Get recommendations based on multiple context keywords."""
        if not context_keywords:
            return []

        context = context_keywords[-max_context_window:]

        for matcher in (self._exact_context_match,
                        self._fuzzy_context_match,
                        self._ngram_fallback):
            recommendations = matcher(context, max_recommendations)
            if recommendations:
                return recommendations
