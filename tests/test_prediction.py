...
class TestPredictNextKeyword:
    ...
    def test_predict_with_valid_context(self, mock_model, sample_tokenizer, sample_keywords):
    ...
    def test_predict_context_longer_than_model_size(self, mock_model, sample_tokenizer, sample_keywords):
        ...
        results = predict_next_keyword(mock_model, sample_tokenizer, long_context, top_k=3)
        
        assert len(results) == 3
        # Should use last context_size keywords
    
    def test_predict_context_shorter_than_model_size(self, mock_model, sample_tokenizer):
        """Test that short context is padded correctly."""
        short_context = ["login1.login_user"]  # Only 1 keyword, model expects 2
        results = predict_next_keyword(mock_model, sample_tokenizer, short_context, top_k=3)
        
        assert len(results) == 3
        # Should pad with empty strings
    
    def test_predict_top_k_parameter(self, mock_model, sample_tokenizer, sample_keywords):
        """Test that top_k parameter controls number of results."""
        context = sample_keywords[0][:2]
        
        results_k1 = predict_next_keyword(mock_model, sample_tokenizer, context, top_k=1)
        results_k5 = predict_next_keyword(mock_model, sample_tokenizer, context, top_k=5)
        
        assert len(results_k1) == 1
        assert len(results_k5) == 5
    
    def test_predict_results_sorted_by_probability(self, mock_model, sample_tokenizer, sample_keywords):
...
class TestPredictNextKeywordWithDepth:
    ...
    def test_predict_depth_1_returns_flat_structure(self, mock_model, sample_tokenizer, sample_keywords):
        """Test that depth=1 returns flat predictions without next_predictions."""
        context = sample_keywords[0][:2]
        results = predict_next_keyword_with_depth(mock_model, sample_tokenizer, context, top_k=3, depth=1)
        
        assert len(results) == 3
        assert all("keyword" in r for r in results)
        assert all("probability" in r for r in results)
        assert all("next_predictions" not in r for r in results)  # No nested predictions
    
    def test_predict_depth_2_returns_hierarchical_structure(self, mock_model, sample_tokenizer, sample_keywords):
        """Test that depth=2 returns hierarchical predictions."""
        context = sample_keywords[0][:2]
        results = predict_next_keyword_with_depth(mock_model, sample_tokenizer, context, top_k=2, depth=2)
    ...
    def test_predict_depth_0_returns_empty_list(self, mock_model, sample_tokenizer, sample_keywords):
        """Test that depth=0 returns empty list."""
        context = sample_keywords[0][:2]
        results = predict_next_keyword_with_depth(mock_model, sample_tokenizer, context, top_k=3, depth=0)
        
        assert results == []
    
    def test_predict_depth_negative_raises_error(self, mock_model, sample_tokenizer, sample_keywords):
        """Test that negative depth raises ValueError."""
        context = sample_keywords[0][:2]
        with pytest.raises(ValueError, match="Depth must be >= 0"):
            predict_next_keyword_with_depth(mock_model, sample_tokenizer, context, top_k=3, depth=-1)
    
    def test_predict_depth_3_recursive_structure(self, mock_model, sample_tokenizer, sample_keywords):
        """Test that depth=3 creates 3 levels of nested predictions."""
        context = sample_keywords[0][:2]
        results = predict_next_keyword_with_depth(mock_model, sample_tokenizer, context, top_k=2, depth=3)
...
class TestLoadModelAndTokenizer:
    ...
    def test_load_model_and_tokenizer_success(self, temp_model_file, temp_tokenizer_file):
        """Test successful loading of model and tokenizer."""
        model, tokenizer = load_model_and_tokenizer(temp_model_file, temp_tokenizer_file)
